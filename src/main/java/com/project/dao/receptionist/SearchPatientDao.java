...
public class SearchPatientDao 
{
	...
	@Transactional
	public Patient searchRamqNo(Long ramqNo) 
	{
		infoLog.logActivities("in SearchPatientDao-searchRAMQNo: got= "+ramqNo);
		
		Session session= sf.getCurrentSession();
		Query q1=session.createQuery("from Patient where ramqNo= :no");
		q1.setParameter("no", ramqNo);
		
		try 
		{
			Patient temp= (Patient) q1.uniqueResult();
			infoLog.logActivities("in SearchPatientDao-searchRAMQNo: found= "+temp);
			return temp;
		}
		catch(Exception e)
		{ 
			infoLog.logActivities("in SearchPatientDao-searchRAMQNo: "+e);
			return null;
		}	
	}
